    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await edit_or_reply(
                xx,
                f"{txt}\n"
                "**Kredensial Heroku tidak valid untuk deploy Alzet-Userbot dyno.**",
            )
            return repo.__del__()
        try:
        ...
        await edit_or_reply(
            xx, "`✧ 𝙰𝙻𝚉𝙴𝚃-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧ Berhasil Di Deploy! Userbot bisa di gunakan kembali.`"
        )
...
async def update(xx, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await edit_or_reply(
        xx, "`✧ 𝙰𝙻𝚉𝙴𝚃-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧ Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
    )
...
async def upstream(event):
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy ✧ 𝙰𝙻𝚉𝙴𝚃-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧ Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**✧ 𝙰𝙻𝚉𝙴𝚃-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧ Sudah Versi Terbaru**")
        return repo.__del__()
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            "**Ketik** `,update deploy` **untuk Mengupdate ✧ 𝙰𝙻𝚉𝙴𝚃-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧ .**"
        )
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}update`\
        \n  •  **Function : **Untuk Melihat Pembaruan Terbaru Alzet-Userbot.\
        \n\n  •  **Syntax :** `{cmd}update deploy`\
        \n  •  **Function : **Untuk MengUpdate Fitur Terbaru Dari Alzet-Userbot.\
    "
    }
)
